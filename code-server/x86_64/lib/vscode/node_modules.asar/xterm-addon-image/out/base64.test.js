...
describe('Base64Decoder', () => {
    describe('decoding', () => {
        it('single bytes', () => {
            const dec = new base64_wasm_1.Base64Decoder(0);
            for (let i = 0; i < 256; ++i) {
                dec.init(1);
                const inp = new Uint8Array([i]);
                const data = fromBs(encNative(inp));
                chai_1.assert.strictEqual(dec.put(data, 0, data.length), 0);
                chai_1.assert.strictEqual(dec.end(), 0);
                chai_1.assert.deepEqual(dec.data8, inp);
            }
        });
        it('1+2 bytes', () => {
            const dec = new base64_wasm_1.Base64Decoder(0);
        ...
        it('padding', () => {
            ...
            for (let i = 0; i < encData.length; ++i) {
                ...
                chai_1.assert.strictEqual(dec.put(enc, 0, enc.length), 0);
                chai_1.assert.strictEqual(dec.end(), 0);
                chai_1.assert.deepEqual(dec.data8, d.slice(0, i + 1));
                // w'o padding
                dec.init(i + 1);
        ...
        it('exit on false byte', () => {
            ...
            for (let pos = 0; pos < 8; ++pos) {
                ...
                for (let i = 0; i < 256; ++i) {
                    ...
                    inp[pos] = i;
                    dec.put(inp, 0, 8);
                    chai_1.assert.strictEqual(dec.end(), MAP.includes(i) ? 0 : 1);
                }
            }
        });
    });
    describe('memory', () => {
        it('always release (keepSize 0)', () => {
            ...
            dec.end();
            chai_1.assert.strictEqual(dec.data8.length, 12);
            dec.release();
            chai_1.assert.strictEqual(dec.data8.length, 0);
            chai_1.assert.isNull(dec._mem);
        });
        it('keep 1 page (keepSize 65536)', () => {
            const dec = new base64_wasm_1.Base64Decoder(65536);
            dec.init(384);
            dec.put(fromBs('A'.repeat(512)), 0, 512);
            dec.end();
            chai_1.assert.strictEqual(dec.data8.length, 384);
            dec.release();
            chai_1.assert.strictEqual(dec.data8.length, 0);
            chai_1.assert.isNotNull(dec._mem);
            // grow to 2 pages + free afterwards
            dec.init(65536);
            dec.put(fromBs('A'.repeat(65536)), 0, 65536);
            dec.end();
            chai_1.assert.strictEqual(dec.data8.length, 49152);
            dec.release();
            chai_1.assert.strictEqual(dec.data8.length, 0);
            chai_1.assert.isNull(dec._mem);
        });
